...
import { Minus, Plus, Save, Sparkles, X } from 'lucide-react';
import { WorkspaceHeader } from '../components/WorkspaceHeader';
import { emailFor, randomInt, randomMobile, randomName, pick } from '../data/autofill';
...
interface FormState {
  ...
  panels: string; galleries: string; workingAreas: string;
  team: TeamMember[];
  rovers: RoverEntry[];
...
export function SiteManagerSetup() {
  ...
  const validate = (): Errors => {
    ...
    if (!positiveInt(form.panels)) e.panels = 'Enter a whole number';
    if (!positiveInt(form.galleries)) e.galleries = 'Enter a whole number';
    if (!positiveInt(form.workingAreas)) e.workingAreas = 'Enter a whole number';
  ...
  const autoFill = () => {
    setForm((f) => ({
      ...f,
      workingMethod: f.workingMethod || pick(WORKING_METHODS),
      gassiness: f.gassiness || pick(GASSINESS),
      levels: f.levels || String(randomInt(1, 4)),
      maxDepth: f.maxDepth || String(randomInt(150, 600)),
      workforce: f.workforce || String(randomInt(40, 150)),
      shifts: f.shifts || '3',
      panels: f.panels || String(randomInt(2, 6)),
      galleries: f.galleries || String(randomInt(2, 6)),
      workingAreas: f.workingAreas || String(randomInt(1, 4)),
      team: f.team.map((m) => ({
        name: m.name || randomName(),
        designation: m.designation || pick(DESIGNATIONS),
        ...
        <form className="access-form" onSubmit={submit} noValidate>
          ...
          <section className="panel">
            ...
            <div className="form-grid">
              ...
              <Field label="NUMBER OF PANELS" required error={errors.panels}><input inputMode="numeric" value={form.panels} onChange={(e) => setField('panels', e.target.value)} /></Field>
              <Field label="NUMBER OF GALLERIES" required error={errors.galleries}><input inputMode="numeric" value={form.galleries} onChange={(e) => setField('galleries', e.target.value)} /></Field>
              <Field label="NUMBER OF WORKING AREAS" required error={errors.workingAreas}><input inputMode="numeric" value={form.workingAreas} onChange={(e) => setField('workingAreas', e.target.value)} /></Field>
            </div>
          </section>
